...
async function extractFeed({ hrefSelector, titleSelector, timeSelector, url }) {
  ...
  console.log(`EXTRACT FEED BASE INFO`, { hrefSelector, titleSelector, timeSelector, url });
  ...
  const feedResult = await page.evaluate(
    (hrefSel, titleSel, timeSel, baseUrl) => {
      ...
      if (timeSel) {
        ...
        times = Array.from(document.querySelectorAll(timeSel)).map((item, index) => {
  ...
  console.log(`FETCH FEED = got finished! feedResult length: ${feedResult.length}`)
